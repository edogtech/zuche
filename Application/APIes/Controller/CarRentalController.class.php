...
class CarRentalController extends Controller {
	...
	public function customerRentalInfo() {
		...
		if (empty($userId)) {
			...
			//$where['o.pay_type'] = array('NEQ','3');//结算完成后订单应该不再显示
			$ob2 = M('e_zc_info');
...
// 			$re=M('e_zc_info')->data($info)->add();//生成租车信息记录,改由支付接口pay生成
    ...
    public function genBalance() {
    	...
    	if (empty($userID)||empty($carID)||empty($TotalMileage)) {
    		...
    		//超时单价（元/时）
    		//$condition['var_name']='excess';
    		//$re_cs=M('e_zc_sysparameter')->field('var_value')->where($condition)->select();
    		...
    		if (empty($re)) {
    			...
    			//$price=$re[0]['price'];//车辆租用单价
    			//$start_time=$re[0]['start_time'];//预约取车时间
    			//$end_time=$re[0]['end_time'];//预约还车时间
    			//$fixed_total_fee=$re[0]['fixed_total_fee'];//预约用车总价
/*定金取消*/
    			//$deposit=$re[0]['deposit'];//车辆租金,数据记录默认为0

    			$return_time=$time;//实际还车时间
				//$borrow_time=$re[0]['borrow_time'];//实际提车时间
				...
				//$excess_use_time=$return_time-$end_time;//超时用车计时 秒
				
				$array=timestap_to_array($borrow_time,$return_time);//时间戳返回天-时-分-秒的数组
				$day=$array['day'];
				$hour=$array['hour'];
				$minute=$array['minute'];
				$second=$array['second'];
...
/*租车价格计算BEGIN*/
				//计算日费用
				$day_fee=$day*$arr['day1'][var_value];//每天148元
				...
				//合计
				$fee=$day_fee+$hour_fee+$minute_fee+$second_fee;
// echo 'dayfee= '.$day_fee,'hourfee= '.$hour_fee,'minutefee= '.$minute_fee,'secondfee= '.$second_fee,'fee= '.$fee;die;

				//判定用车时间段是否在夜间套餐范围
				//租车时间"17点-19点";还车时间"17点-24点"或"0点-8点半"，共15个半小时不足1天
	    		$borrow_hour=date('His',$borrow_time);
				$return_hour=date('His',$return_time);

				if($day==0){
					if(($borrow_hour>='170000' && $borrow_hour<='190000')){
						if(($return_hour>='170000' && $return_hour<='235959')||($return_hour>='000000' && $return_hour<='083000')){
							//echo '享受晚间套餐啦';
							$fee=$arr['evening'][var_value];//晚间套餐100元
						}
					}
				}
				...
				$payment=$fee;//折扣后实付金额
//echo $arr['discount'][var_value],'<br>';

/*租车价格计算END*/
				
    			//$payment=$actual_total_fee-$deposit;//实付金额

    			//生成结算订单记录 因不再缴纳预支付定金，故仍用orders表结算
    			//$balance_number=$re[0]['order_number'].'JS';//结算订单号
    			//$text['order_id']=$reInfo[0]['order_id'];//预支付订单ID
    			//$text['balance_number']=$balance_number;
    			//$text['payment']=$payment;//结算金额
    			//$text['pay_type']=0;//待结算
    			//$balance_id=M('e_zc_balance')->data($text)->add();//生成结算订单
    			...
    			// if ($flag) {
					// $data['excess_use_time']=$excess_use_time;//超时用车计时
    				// $data['excess']=$re_cs[0]['var_value'];//超时单价
					// $data['excess_fee']=$overtime;//超时金额
    			// }
