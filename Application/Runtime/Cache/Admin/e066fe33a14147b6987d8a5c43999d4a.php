...
<!-- 	<li class="dropdown">
	...
	<div class="row">
		<div class="col-md-6">
			<form action="<?php echo U('Car/do_modify_model');?>" method="post" enctype="multipart/form-data">
				<div class="form-group">
					<label>现有的品牌</label>
					<select class="form-control" name="brand_id">

						<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php echo $data1['brand_id']==$vo['id']?'selected':'';;?>><?php echo ($vo['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				...
				<div class="form-group">
					<label>型号名称</label>
					<input class="form-control" name="name" value="<?php echo ($data1['name']); ?>">
					<p class="help-block">请输入型号名称</p>
				</div>
				<div class="form-group">
					<label>车型图片</label>
					<img  class="img-responsive" src="/zuche<?php echo ($data1['img_url']); ?>"/>
				</div>
				<div class="form-group">
					<label>修改后的图片</label>
					<input type="file" id="upload" name="img_url">
				</div>
				<div class="form-group">
					<label>修改图片预览</label>
					<img id="ImgPr" class="img-responsive" />
				</div>
