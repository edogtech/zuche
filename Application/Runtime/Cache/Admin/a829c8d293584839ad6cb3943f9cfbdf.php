...
<!-- 	<li class="dropdown">
	...
	<div class="row">
		<div class="col-md-8" style="margin-bottom:20px;">
			<button type="button" class="btn btn-primary" onclick="location.href='<?php echo U('AdminUser/do_save',array('table'=>'all'));?>'">备份库到服务器</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<button type="button" class="btn btn-warning" onclick="location.href='<?php echo U('AdminUser/download',array('table'=>'all'));?>'">下载库到本地</button>
		</div>
	</div>
	<div class="row">
		<!-- /.panel-heading -->
						...
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								<?php if(is_array($table)): $i = 0; $__LIST__ = $table;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										<td><?php echo ($vo['Tables_in_yineng']); ?></td>
										<td>
											<button type="button" class="btn btn-primary btn-xs" onclick="location.href='<?php echo U('AdminUser/do_save',array('table'=>$vo['Tables_in_yineng']));?>'">备份到服务器</button>
											<button type="button" class="btn btn-warning btn-xs" onclick="location.href='<?php echo U('AdminUser/download',array('table'=>$vo['Tables_in_yineng']));?>'">下载到本地</button>
										</td>
									</tr><?php endforeach; endif; else: echo "" ;endif; ?>
							</tbody>
						</table>
						<?php if(empty($table)): ?><div style="text-align:center;">抱歉，暂时没有您要找的内容</div><?php endif; ?>
					</div>
				</div>
			</div>
			<!-- 分页内容 -->
