...
<!-- 	<li class="dropdown">
    ...
    <div class="row">
        <form method="get" action="<?php echo U('Car/car');?>">
        <div class="col-md-3">
            <div class="form-group">
                品牌:
                <select style="display:inline;width:70%;" class="form-control" name="brand">
                    
                    <option value="" <?php echo empty($_GET['brand'])?"selected":'';;?> >全部</option>

                    <?php if(is_array($brand)): $i = 0; $__LIST__ = $brand;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php echo $_GET['brand']==$vo['id']?"selected":'';;?> ><?php echo ($vo['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="col-md-3">
            <div class="form-group">
                型号:
                <select style="display:inline;width:70%;" class="form-control" name="model">
                    
                    <option value="" <?php echo empty($_GET['model'])?"selected":'';;?> >全部</option>

                    <?php if(is_array($model)): $i = 0; $__LIST__ = $model;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php echo $_GET['model']==$vo['id']?"selected":'';;?> ><?php echo ($vo['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="col-md-3">
            <div class="form-group">
                站点:
                <select style="display:inline;width:70%;" class="form-control" name="station">
                    
                    <option value="" <?php echo empty($_GET['station'])?"selected":'';;?> >全部</option>

                    <?php if(is_array($station)): $i = 0; $__LIST__ = $station;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php echo $_GET['station']==$vo['id']?"selected":'';;?> ><?php echo ($vo['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="col-md-3">
            <div class="form-group">
                车牌号:
                <input style="display:inline;width:70%;" class="form-control" type="text" name="plate" value="<?php echo isset($_GET['plate'])?$_GET['plate']:'';;?>"></div>
        </div>
        <div class="col-md-3 col-md-offset-9" >
            <div class="form-group">
                ...
                <button type="button" class="btn btn-primary btn-md" onclick="location.href='<?php echo U('Car/add_car');?>'">添加车辆</button>
            </div>
        </div>
        </form>
    </div>
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary btn-xs" onclick="location.href='<?php echo U('Car/modify_car',array('id'=>$vo['id']));?>'">修改</button>
                                            <button type="button" data-id="<?php echo ($vo['id']); ?>" data-name="<?php echo ($vo['name']); ?>" data-target="#delete_brand" data-toggle="modal" class="btn btn-danger btn-xs" >删除</button>
                                        </td>
                                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>

                            </tbody>
                        </table>
                        <?php if(empty($data)): ?><div style="text-align:center;">抱歉，暂时没有您要找的内容</div><?php endif; ?>
                    </div>
    ...
    <!--删除模态框-->
    <div class="modal fade" id="delete_brand" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
        <form method="post" action="<?php echo U('Car/del_car');?>">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="exampleModalLabel">删除车辆</h4>
                    </div>
                    <div class="modal-body">

                        <div class="form-group">
                            <h4 style="text-align:center;color:red;" ><b id="del_brand_name"></b></h4>
                            <p style="color:orange;text-align:center;">确定删除此车辆信息？</p>
                        </div>

                    </div>
                    <div class="modal-footer">
                        <input class="form-control" type="hidden" name="id" id="del_brand_id"  />
