...
<!-- 	<li class="dropdown">
	...
	<div class="row">
		<form method="get" action="<?php echo U('Carousel/index');?>">
			<div class="col-md-3">
				<div class="form-group">
					标题:
					<input class="form-control" style="display:inline;width:70%;" type="text" name="caption" id="caption" value="<?php echo isset($_GET['caption'])?$_GET['caption']:'';;?>"></div>
			</div>		
			<div class="col-md-6">
				...
				<table class="table table-striped table-bordered table-hover">
					...
					<tbody>
						<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><img  class="img-responsive"  src="/zuche<?php echo ($vo['image']); ?>" style="width:50px;height:30px;margin:0 auto;"/></td>
							<td><?php echo ($vo['url']); ?></td>
							<td><?php echo ($vo['summary']); ?></td>
							<!-- <td>
								<?php if(($vo['type']) == "0"): ?>--<?php else: ?>外链<?php endif; ?>
							</td> -->								
							<td><?php echo (date('Y-m-d',$vo['add_time'])); ?></td>								
							<td align="center">
								<a href="#" data-toggle="modal" data-target="#CarouselDelModal" data-id="<?php echo ($vo['id']); ?>">删除</a>
								<a href="#" data-toggle="modal" data-target="#CarouselEditModal"
								 data-id="<?php echo ($vo['id']); ?>"
								 data-caption="<?php echo ($vo['name']); ?>"
								 data-url="<?php echo ($vo['url']); ?>"
								 data-summary="<?php echo ($vo['summary']); ?>"
								 data-image="/zuche<?php echo ($vo['image']); ?>">编辑</a>
							</td>
	...
	<div class="modal fade" id="CarouselAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<form method="post" enctype="multipart/form-data" action="<?php echo U('Carousel/add');?>">
	...
	<div class="modal fade" id="CarouselDelModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<form method="post" action="<?php echo U('Carousel/del');?>">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"
	...
	<div class="modal fade" id="CarouselEditModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<form method="post" enctype="multipart/form-data" action="<?php echo U('Carousel/edit');?>">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"
